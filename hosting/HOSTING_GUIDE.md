...
the repo root - the folder containing `redteam/`, `vendor/`, `tests/`, and this
`hosting/` folder. That whole folder is the GitHub repo, and both `app.py` and
`requirements.txt` live at its root.
...
Open the repo root `.gitignore` and
...
`.env`. That file must never be committed - the repo ships
`.env.example` instead (variable names, no values), and
...
roadmap: `vendor/data/company.db` IS checked in. It's a tiny
...
Refresh the repo page - you should see `redteam/`, `vendor/`, `tests/`,
`hosting/`, `app.py`, and this project's root `README.md`.
...
- `app.py` - copy from the repo root.
...
- `redteam/` - the package itself, copied wholesale from `redteam/`.
- `vendor/` - copied wholesale from `vendor/`, INCLUDING
...
copy ..\llm-redteam-jailbreak-benchmark-github\app.py .
copy ..\llm-redteam-jailbreak-benchmark-github\hosting\space\requirements.txt .
copy ..\llm-redteam-jailbreak-benchmark-github\hosting\space\README.md .
xcopy ..\llm-redteam-jailbreak-benchmark-github\redteam redteam\ /E /I /EXCLUDE:nul
xcopy ..\llm-redteam-jailbreak-benchmark-github\vendor vendor\ /E /I /EXCLUDE:nul
...
(Adjust the `..\llm-redteam-jailbreak-benchmark-github\` prefix to wherever your
...
3. Set **Main file path** to `app.py` - it sits at the repo root.
4. Deploy. Community Cloud looks for a `requirements.txt` next to the app file
   it's running, so it picks up `requirements.txt`
...
because the whole repo is already exactly what the app
